 ...
 * @since 1.0
 */
public class ActivityListIntenational extends BaseFragment implements ImlAirlineInfo.View{
  /*  @Override
    public int setContentViewId() {
        return R.layout.activity_airline;
    }*/
  public static ActivityListIntenational fragment;
    public static synchronized ActivityListIntenational getInstance() {
        if (fragment == null) {
            fragment = new ActivityListIntenational();
        }
        return (fragment);
    }
        ...
        if (listAirline!=null&&listAirline.size()>0){
          ...
          //  App.lisAirline.addAll(listAirline);
            for (AirlineInfo obj:listAirline){
                if (obj.getsAIRLINE_TYPE().equals("2")){
                    mLisFlight.add(obj);
                }
            }
