...
import neo.com.noibai_airport.Adapter.AdapterListDistrict;
...
public class ActivityListDistrict extends BaseActivity implements ImlTogether.View {
    ...
    private void initData() {
        if (App.lisDistrict != null && App.lisDistrict.size() > 0) {
            mLisAirport.addAll(App.lisDistrict);
            temp.addAll(mLisAirport);
            adapterService.notifyDataSetChanged();
        } else {
            sUserId = SharedPrefs.getInstance().get(Constants.KEY_USERID, String.class);
            showDialogLoading();
            mPresenter.api_get_list_district(sUserId, "1");
        }
    }
