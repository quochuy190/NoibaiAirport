...
import neo.com.noibai_airport.Adapter.AdapterListAirport_Search;
...
public class ActivityListAirport_Search extends BaseActivity implements ImlAirlineInfo.View {

    private List<Airport> mLisAirport;
    private AdapterListAirport_Search adapterService;
    ...
    void filter(String text) {
        ...
        adapterService.setOnIListener(new setOnItemClickListener() {
            ...
            public void OnItemClickListener(int position) {
                App.mAirportSearch = temp.get(position);
                finish();
            }
    ...
    private void init() {
        ...
        temp = new ArrayList<>();
        adapterService = new AdapterListAirport_Search(temp, this);
        ...
        adapterService.setOnIListener(new setOnItemClickListener() {
            @Override
            public void OnItemClickListener(int position) {
                App.mAirportSearch = mLisAirport.get(position);
                finish();
    ...
    private void initData() {
        if (App.lisAirport != null && App.lisAirport.size() > 0) {
            mLisAirport.addAll(App.lisAirport);
            temp.addAll(mLisAirport);
            adapterService.notifyDataSetChanged();
        } else {
            sUserId = SharedPrefs.getInstance().get(Constants.KEY_USERID, String.class);
            showDialogLoading();
            mPresenter.get_all_airport(sUserId);
        }
    }
    ...
    public void show_list_airport(List<Airport> lisAirport) {
        hideDialogLoading();
        if (lisAirport != null) {
            mLisAirport.addAll(lisAirport);
            temp.addAll(mLisAirport);
            App.lisAirport.clear();
            App.lisAirport.addAll(mLisAirport);
            adapterService.notifyDataSetChanged();
        }
    }
