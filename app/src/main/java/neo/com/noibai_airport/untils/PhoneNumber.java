...
public class PhoneNumber {
    public static String add_multil_phone(List<PhoneContactModel> lisPhone) {
        String phone = "";
        if (lisPhone.size() == 1) {
            phone = lisPhone.get(0).getPhoneNumber();
            return phone;
        } else if (lisPhone.size() > 1) {
            for (int i = 0; i < lisPhone.size(); i++) {
                if (i < (lisPhone.size() - 1))
                    phone = phone + lisPhone.get(i).getPhoneNumber() + ",";
                else
                    phone = phone + lisPhone.get(i).getPhoneNumber();
            }
            return phone;
